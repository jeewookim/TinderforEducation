    ...
    private void attemptLogin(SharedPreferences.Editor editor) {
        ...
        if (cancel) {
            ...
            if(!mSettings.contains(email)){
                User person = new User(email);
                person.setPassword(password);
                totalusers.add(person);
                for (int i = 0; i < totalusers.size(); i++)
                {
                    Log.i("names" + (totalusers.get(i)).getEmail(), "size" + totalusers.size());
                }
                editor.putString(email, person.getEmail() + " " + person.getName() + " " + person.getStatus() + " " + person.getSubject() + " " + person.getAvail() + " " + person.getSize());
                editor.commit();
            }
            ...
            //need to traverse through all shared preferences keys, and make totalusers by splitting up the long ass string^^
            ...
            intent.putExtra("key", totalusers);
